    ...
    for(tt = 0; tt < settings->max_iters; ++tt)
    {
        ...
        else 
        {
            ...
            if(num_cheby_iters == 1)
            {
                ...
                double bb = 0.0;
                cheby_solver_init_driver(chunks, settings, tt, &bb);

                // Perform the main step
                cheby_solver_main_step_driver(
        ...
        Chunk* chunks, Settings* settings, int num_cg_iters, double* bb)
{
    *bb = 0.0;
    ...
    cheby_solver_coef_driver(chunks, settings, settings->max_iters-num_cg_iters);

    for(int cc = 0; cc < settings->num_chunks_per_rank; ++cc)
    {
        if(settings->kernel_language == C)
        {
            run_calculate_2norm(&(chunks[cc]), settings, chunks[cc].vec_u0, bb);

            run_cheby_solver_init(&(chunks[cc]), settings);
        }
