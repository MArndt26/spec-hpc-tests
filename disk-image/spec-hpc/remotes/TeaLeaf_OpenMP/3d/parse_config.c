...
void read_config(Settings* settings, State** states)
{
    ...
    for(int ss = 0; ss < settings->num_states; ++ss)
    {
        ...
        if(ss > 0)
        {
            ...
            print_to_log(settings, "\tz_min = %.12E\n", (*states)[ss].z_min);
            print_to_log(settings, "\tx_max = %.12E\n", (*states)[ss].x_max);
            print_to_log(settings, "\ty_max = %.12E\n", (*states)[ss].y_max);
            print_to_log(settings, "\tz_max = %.12E\n", (*states)[ss].z_max);
...
void read_settings(FILE* tea_in, Settings* settings)
{
    ...
    settings->dz = (settings->grid_z_max-settings->grid_z_min) / 
        (double)settings->grid_z_cells;
}
    ...
    while(getline(&line, &len, tea_in) != EOF)
    {
        ...
        if(starts_get_int("state", line, word, &state_num))
        {
            ...
            if(state_num > 1)
            {
                ...
                read_value(line, "zmin", word); 
                state->z_min = atof(word) + settings->dz/100.0;
                ...
                read_value(line, "zmax", word); 
                state->z_max = atof(word) - settings->dz/100.0;

                read_value(line, "geometry", word);

                if(strmatch(word, "cuboid"))
                {
                    state->geometry = CUBOID;
                }
                else if(strmatch(word, "spherical"))
                {
                    state->geometry = SPHERICAL;
