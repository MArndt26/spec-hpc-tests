...
{
    set_chunk_data(
            settings, chunk->x, chunk->y, chunk->left, 
            chunk->bottom, chunk->cell_x, chunk->cell_y, chunk->vertex_x, 
            chunk->vertex_y, chunk->volume, chunk->x_area, chunk->y_area);
}
...
void run_set_chunk_state(Chunk* chunk, Settings* settings, State* states)
{
    set_chunk_state(
            ...
            chunk->vertex_y, chunk->cell_x, chunk->cell_y, chunk->density, 
            chunk->energy0, chunk->u, settings->num_states, states);
}

void run_kernel_initialise(Chunk* chunk, Settings* settings)
{
    kernel_initialise(
            settings, &(chunk->ext->raja_lists), chunk->ext->mem_pool, chunk->x, 
            chunk->y, &(chunk->density0), &(chunk->density), &(chunk->energy0), 
...
{
    kernel_finalise(
            chunk->ext->mem_pool, chunk->density0, chunk->density, chunk->energy0, 
            chunk->energy, chunk->u, chunk->u0, chunk->p, chunk->r, chunk->mi, 
            chunk->w, chunk->kx, chunk->ky, chunk->sd, chunk->volume, chunk->x_area,
            ...
            chunk->cheby_alphas, chunk->cheby_betas, chunk->ext->raja_lists);
}
        ...
        Chunk* chunk, Settings* settings, int depth)
{
    ...
    local_halos(
            chunk->ext->raja_lists, chunk->x, chunk->y, depth, 
            settings->halo_depth, chunk->neighbours, 
        ...
        double* vol, double* mass, double* ie, double* temp)
{
    ...
    field_summary(
            ...
            chunk->volume, chunk->density, chunk->energy0, chunk->u, 
            vol, mass, ie, temp);
    STOP_PROFILING(settings->kernel_profile, __func__);
}
        ...
        Chunk* chunk, Settings* settings, double alpha, double* rrn)
{
    ...
    cg_calc_ur(
            chunk->ext->raja_lists, chunk->x, chunk->y, settings->halo_depth, 
            alpha, rrn, chunk->u, chunk->p, chunk->r, chunk->w);
    STOP_PROFILING(settings->kernel_profile, __func__);
}
        ...
        Chunk* chunk, Settings* settings, double* error)
{
    ...
    jacobi_iterate(
            chunk->ext->raja_lists, chunk->x, chunk->y, settings->halo_depth, 
            error, chunk->kx, chunk->ky, chunk->u0, chunk->u, chunk->r);
    STOP_PROFILING(settings->kernel_profile, __func__);
}
...
void run_calculate_residual(Chunk* chunk, Settings* settings)
{
    ...
    calculate_residual(
            chunk->ext->raja_lists, chunk->x, chunk->y, settings->halo_depth, 
            chunk->u, chunk->u0, chunk->r, chunk->kx, chunk->ky);
    STOP_PROFILING(settings->kernel_profile, __func__);
}
...
void run_finalise(Chunk* chunk, Settings* settings)
{
    ...
    finalise(
            chunk->ext->raja_lists, chunk->x, chunk->y, settings->halo_depth, 
            chunk->energy, chunk->density, chunk->u);
    STOP_PROFILING(settings->kernel_profile, __func__);
}
