...
void solve(Chunk* chunks, Settings* settings, int tt, double* wallclock_prev)
{
    ...
    double rz = dt / (settings->dz*settings->dz);
    ...
    {
        case JACOBI_SOLVER:
            jacobi_solver_driver(chunks, settings, rx, ry, rz, &error);
            break;
        case CG_SOLVER:
            cg_solver_driver(chunks, settings, rx, ry, rz, &error);
            break;
        case CHEBY_SOLVER:
            cheby_solver_driver(chunks, settings, rx, ry, rz, &error);
            break;
        case PPCG_SOLVER:
            ppcg_solver_driver(chunks, settings, rx, ry, rz, &error);
            break;
    }
