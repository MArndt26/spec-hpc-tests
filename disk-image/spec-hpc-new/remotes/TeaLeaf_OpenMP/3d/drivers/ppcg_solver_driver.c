...
void ppcg_solver_inner_iterations(
        Chunk* chunks, Settings* settings);

// Performs a full solve with the PPCG solver
void ppcg_solver_driver(Chunk* chunks, Settings* settings,
        double rx, double ry, double rz, double* error)
{
    ...
    cg_solver_init_driver(chunks, settings, rx, ry, rz, &rro);
    ...
    for(tt = 0; tt < settings->max_iters; ++tt)
    {
        ...
        if(!is_switch_to_ppcg)
        {
            ...
            cg_solver_main_step_driver(chunks, settings, tt, &rro, error);
        }
        else 
        {
            num_ppcg_iters++;
            ...
            if(num_ppcg_iters == 1)
            {
                ...
                cheby_solver_coef_driver(
                       chunks, settings, settings->ppcg_inner_steps);

                ppcg_solver_init_driver(chunks, settings, &rro);
            }

            ppcg_solver_main_step_driver(chunks, settings, &rro, error);
        }
...
void ppcg_solver_init_driver(Chunk* chunks, Settings* settings, double* rro)
{
        ...
        Chunk* chunks, Settings* settings, double* rro, double* error)
{
    ...
    for(int cc = 0; cc < settings->num_chunks_per_rank; ++cc)
    {
        if(settings->kernel_language == C)
        {
            run_cg_solver_calc_w(&(chunks[cc]), settings, &pw);
        }
        else if(settings->kernel_language == FORTRAN)
        {
        }
    ...
    for(int cc = 0; cc < settings->num_chunks_per_rank; ++cc)
    {
        if(settings->kernel_language == C)
        {
            run_cg_solver_calc_ur(&(chunks[cc]), settings, alpha, &rrn);
        }
        else if(settings->kernel_language == FORTRAN)
        {
        }
    ...
    ppcg_solver_inner_iterations(chunks, settings);
    ...
    for(int cc = 0; cc < settings->num_chunks_per_rank; ++cc)
    {
        ...
        {
            run_calculate_2norm(
                    &(chunks[cc]), settings, chunks[cc].vec_r, &rrn);
        }
    }
    ...
    for(int cc = 0; cc < settings->num_chunks_per_rank; ++cc)
    {
        if(settings->kernel_language == C)
        {
            run_cg_solver_calc_p(&(chunks[cc]), settings, beta);
        }
        else if(settings->kernel_language == FORTRAN)
        {
        }
