...
# output.
command = (
    "echo 'System Booted: switching from KVM cpu to test cpu';"
    + "m5 exit;"
    + "echo 'Now running on Timing cpu';"
    + "sleep 1;"
    + "m5 exit;"
    + "echo 'Run Complete';"
)
...
board.set_kernel_disk_workload(
    kernel=Resource(
        resource_name="x86-linux-kernel-5.4.49",
        resource_directory="disk-image/prebuilt",  # relative to gem5 folder
    ),
    disk_image=Resource(
        resource_name="x86-ubuntu-18.04-img",
        resource_directory="disk-image/prebuilt",  # relative to the gem5 folder
    ),
    readfile_contents=command,
)


def on_switch_callback():
    print("switching processor")
    processor.switch()
...
simulator = Simulator(
    ...
    on_exit_event={
        ...
        ExitEvent.EXIT: (func() for func in [on_switch_callback]),
    },
)
simulator.run()
